...
export const VolunteerNotification: React.FC<VolunteerNotificationProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await fetch(apiUrl(`/api/notifications/${currentUser.id}?role=volunteer&limit=20`));
        if (!response.ok) {
  ...
  const respondToNotification = async (notification: any, response: 'accepted' | 'declined') => {
    ...
    if (socket?.connected) {
      ...
      await fetch(apiUrl(`/api/notifications/${notification.notificationId}/respond`), {
        method: 'PUT',
