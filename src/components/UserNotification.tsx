...
export const UserNotification: React.FC<UserNotificationProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await fetch(apiUrl(`/api/notifications/${currentUser.id}?role=user&limit=100`));
  ...
  const markAsRead = async (notificationId: string) => {
    ...
    await fetch(apiUrl(`/api/notifications/${notificationId}/read`), { method: 'PUT' }).catch(() => undefined);
  };
